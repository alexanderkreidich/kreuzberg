...
impl ExtractionConfig {
    ...
    fn effective_disable_ocr(&self) -> bool {
        #[allow(clippy::needless_update)]
        let core_self = kreuzberg::ExtractionConfig {
            use_cache: self.use_cache,
            enable_quality_processing: self.enable_quality_processing,
...
impl OcrConfig {
    ...
    fn effective_thresholds(&self) -> OcrQualityThresholds {
        let core_self = kreuzberg::OcrConfig {
            enabled: self.enabled,
            backend: self.backend.clone(),
            language: self.language.clone(),
    ...
    fn effective_pipeline(&self) -> Option<OcrPipelineConfig> {
        let core_self = kreuzberg::OcrConfig {
            enabled: self.enabled,
            backend: self.backend.clone(),
            language: self.language.clone(),
...
impl ChunkingConfig {
    ...
    fn with_chunker_type(&self, chunker_type: ChunkerType) -> ChunkingConfig {
        panic!("alef: with_chunker_type not auto-delegatable")
    }

    fn with_sizing(&self, sizing: ChunkSizing) -> ChunkingConfig {
        panic!("alef: with_sizing not auto-delegatable")
    }

    fn with_prepend_heading_context(&self, prepend: bool) -> ChunkingConfig {
    ...
    fn with_topic_threshold(&self, threshold: f32) -> ChunkingConfig {
        let core_self = kreuzberg::ChunkingConfig {
            max_characters: self.max_characters,
            overlap: self.overlap,
            trim: self.trim,
...
impl ServerConfig {
    ...
    fn apply_env_overrides(&self) -> Result<(), Error> {
        let core_self = kreuzberg::ServerConfig {
            host: self.host.clone(),
            port: self.port,
            cors_origins: self.cors_origins.clone(),
...
unsafe impl IntoValueFromNative for OcrFallbackDecision {}

impl magnus::TryConvert for OcrFallbackDecision {
    fn try_convert(val: magnus::Value) -> Result<Self, magnus::Error> {
        let r: &OcrFallbackDecision = magnus::TryConvert::try_convert(val)?;
        Ok(r.clone())
    }
}
...
impl OcrFallbackDecision {
    ...
    fn avg_non_whitespace(&self) -> f64 {
        self.avg_non_whitespace
    }

    fn avg_alnum(&self) -> f64 {
        self.avg_alnum
    }
...
impl DocumentStructure {
    ...
    fn push_node(&self, node: DocumentNode) -> u32 {
        0
    }

    fn add_child(&self, parent: u32, child: u32) -> () {
...
impl ImagePreprocessingMetadata {
    ...
    fn original_dimensions(&self) -> String {
        self.original_dimensions.clone()
    }

    fn original_dpi(&self) -> String {
    ...
    fn new_dimensions(&self) -> Option<String> {
...
impl OcrElement {
    ...
    fn with_level(&self, level: OcrElementLevel) -> OcrElement {
...
impl HierarchicalBlock {
    fn new(text: String, font_size: f32, level: String, bbox: Option<String>) -> Self {
        Self {
            text,
            font_size,
            level,
...
impl Uri {
    ...
    fn with_page(&self, page: u32) -> Uri {
...
impl KeywordConfig {
    ...
    fn with_max_keywords(&self, max: usize) -> KeywordConfig {
        #[allow(clippy::needless_update)]
    ...
    fn with_min_score(&self, score: f32) -> KeywordConfig {
        #[allow(clippy::needless_update)]
        let core_self = kreuzberg::KeywordConfig {
            algorithm: self.algorithm.clone().into(),
            max_keywords: self.max_keywords,
    ...
    fn with_ngram_range(&self, min: usize, max: usize) -> KeywordConfig {
        #[allow(clippy::needless_update)]
        let core_self = kreuzberg::KeywordConfig {
            algorithm: self.algorithm.clone().into(),
            max_keywords: self.max_keywords,
    ...
    fn with_language(&self, lang: String) -> KeywordConfig {
        #[allow(clippy::needless_update)]
        let core_self = kreuzberg::KeywordConfig {
            algorithm: self.algorithm.clone().into(),
            max_keywords: self.max_keywords,
...
impl magnus::TryConvert for FontSizeCluster {
    fn try_convert(val: magnus::Value) -> Result<Self, magnus::Error> {
        let r: &FontSizeCluster = magnus::TryConvert::try_convert(val)?;
        Ok(r.clone())
    }
}
...
unsafe impl IntoValueFromNative for CharData {}

impl magnus::TryConvert for CharData {
    fn try_convert(val: magnus::Value) -> Result<Self, magnus::Error> {
        let r: &CharData = magnus::TryConvert::try_convert(val)?;
        Ok(r.clone())
    }
}
unsafe impl TryConvertOwned for CharData {}
...
impl magnus::TryConvert for HierarchyBlock {
    fn try_convert(val: magnus::Value) -> Result<Self, magnus::Error> {
        let r: &HierarchyBlock = magnus::TryConvert::try_convert(val)?;
        Ok(r.clone())
    }
}
...
fn is_cache_valid(cache_path: String, max_age_days: f64) -> bool {
...
fn generate_cache_key(parts: Vec<String>) -> String {
...
fn filter_old_cache_entries(cache_times: Vec<f64>, current_time: f64, max_age_seconds: f64) -> Vec<usize> {
    kreuzberg::cache::filter_old_cache_entries(&cache_times, current_time, max_age_seconds)
}

fn sort_cache_by_access_time(entries: Vec<String>) -> Vec<String> {
    Vec::new()
}

fn sanitize_namespace(namespace: String) -> Option<String> {
...
fn resolve_thread_budget(config: Option<String>) -> usize {
    0
}

fn init_thread_pools(budget: usize) -> () {
...
fn validate_vlm_backend_config(backend: String, vlm_config: Option<String>) -> Result<(), Error> {
    let vlm_config: Option<LlmConfig> = vlm_config
        .as_deref()
        .filter(|s| *s != "nil")
        .map(|s| {
...
fn apply_output_format(result: String, output_format: String) -> ExtractionResult {
    ...
    panic!("alef: apply_output_format not auto-delegatable")
}
...
fn resolve_relationships(doc: String) -> () {
    ()
}
...
fn estimate_html_markdown_capacity(html_size: u64) -> usize {
    kreuzberg::extraction::estimate_html_markdown_capacity(html_size)
}

fn estimate_spreadsheet_capacity(file_size: u64) -> usize {
    kreuzberg::extraction::estimate_spreadsheet_capacity(file_size)
}

fn estimate_presentation_capacity(file_size: u64) -> usize {
    kreuzberg::extraction::estimate_presentation_capacity(file_size)
}

fn estimate_table_markdown_capacity(row_count: usize, col_count: usize) -> usize {
...
fn collect_and_convert_omath(reader: String) -> String {
...
fn extract_text_from_events(events: Vec<String>) -> String {
...
fn extract_text_from_proto(data: Vec<u8>) -> Vec<String> {
...
fn extract_metadata_from_zip(content: Vec<u8>) -> Metadata {
...
fn extract_pict_image(chars: String) -> String {
    String::from("[unimplemented: extract_pict_image]")
}

fn parse_rtf_datetime(segment: String) -> Option<String> {
    kreuzberg::extractors::rtf::parse_rtf_datetime(&segment)
}

fn extract_rtf_metadata(rtf_content: String, extracted_text: String) -> String {
...
fn extract_panic_message(panic_info: String) -> String {
...
fn render_djot(doc: String) -> String {
    String::from("[unimplemented: render_djot]")
}

fn render_html(doc: String) -> String {
    String::from("[unimplemented: render_html]")
}

fn render_json(doc: String) -> String {
    String::from("[unimplemented: render_json]")
}

fn render_markdown(doc: String) -> String {
    String::from("[unimplemented: render_markdown]")
}

fn render_plain(doc: String) -> String {
...
fn extractor_span(extractor_name: String, mime_type: String, size_bytes: usize) -> String {
    String::from("[unimplemented: extractor_span]")
}

fn pipeline_stage_span(stage: String) -> String {
    String::from("[unimplemented: pipeline_stage_span]")
}

fn pipeline_processor_span(stage: String, processor_name: String) -> String {
    String::from("[unimplemented: pipeline_processor_span]")
}

fn ocr_span(backend: String, language: String) -> String {
    String::from("[unimplemented: ocr_span]")
}

fn model_inference_span(model_name: String) -> String {
...
fn get_reduction_statistics(original: String, reduced: String) -> String {
...
fn snake_to_camel(val: String) -> String {
    String::from("[unimplemented: snake_to_camel]")
}

fn camel_to_snake(val: String) -> String {
    String::from("[unimplemented: camel_to_snake]")
}

fn is_markdown_header(line: String) -> bool {
...
fn map_kreuzberg_error_to_mcp(error: String) -> String {
...
fn detect_plain_text_boundaries(text: String) -> Vec<DetectedBoundary> {
...
fn chunk_semantic(
    ...
    let result = kreuzberg::chunking::semantic::chunk_semantic(&text, config.into(), page_boundaries.as_deref())
        .map_err(|e| {
            magnus::Error::new(
                unsafe { Ruby::get_unchecked() }.exception_runtime_error(),
                e.to_string(),
...
fn precompute_utf8_boundaries(text: String) -> String {
...
fn warm_model(model_type: String, cache_dir: Option<String>) -> Result<(), Error> {
    ...
    let result = kreuzberg::warm_model(model_type.into(), cache_dir.map(std::path::PathBuf::from)).map_err(|e| {
        magnus::Error::new(
            unsafe { Ruby::get_unchecked() }.exception_runtime_error(),
            e.to_string(),
        )
...
fn download_model(model_type: String, cache_dir: Option<String>) -> Result<(), Error> {
    ...
    let result =
        kreuzberg::download_model(model_type.into(), cache_dir.map(std::path::PathBuf::from)).map_err(|e| {
            magnus::Error::new(
                unsafe { Ruby::get_unchecked() }.exception_runtime_error(),
                e.to_string(),
            )
...
fn get_stopwords(lang: String) -> Option<String> {
    None
}

fn get_stopwords_with_fallback(language: String, fallback: String) -> Option<String> {
    None
}
...
fn is_language_supported(lang: String) -> bool {
    kreuzberg::paddle_ocr::is_language_supported(&lang)
}

fn language_to_script_family(paddle_lang: String) -> String {
    kreuzberg::paddle_ocr::language_to_script_family(&paddle_lang).into()
}

fn map_language_code(kreuzberg_code: String) -> Option<String> {
...
fn preprocess_imagenet_letterbox(img: String, target_size: u32) -> String {
    String::from("[unimplemented: preprocess_imagenet_letterbox]")
}

fn preprocess_rescale(img: String, target_size: u32) -> String {
    String::from("[unimplemented: preprocess_rescale]")
}

fn preprocess_letterbox(img: String, target_width: u32, target_height: u32) -> String {
...
fn take_or_create_tatr(accel: Option<String>) -> Option<String> {
...
fn return_tatr(model: String) -> () {
    ()
}

fn take_or_create_slanet(variant: String, accel: Option<String>) -> Option<String> {
    let accel: Option<AccelerationConfig> = accel
        .as_deref()
        .filter(|s| *s != "nil")
        .map(|s| {
...
fn return_slanet(variant: String, model: String) -> () {
    ()
}

fn take_or_create_table_classifier(accel: Option<String>) -> Option<String> {
    let accel: Option<AccelerationConfig> = accel
        .as_deref()
        .filter(|s| *s != "nil")
        .map(|s| {
...
fn return_table_classifier(model: String) -> () {
    ()
}

fn extract_annotations_from_document(document: String) -> Vec<PdfAnnotation> {
    Vec::new()
}
...
impl From<NodeContent> for kreuzberg::NodeContent {
    fn from(val: NodeContent) -> Self {
        match val {
            ...
            NodeContent::Table { grid } => Self::Table { grid },
            NodeContent::Image {
                description,
                image_index,
                src,
...
fn init(ruby: &Ruby) -> Result<(), Error> {
    ...
    class.define_method(
        ...
        method!(ExtractionConfig::effective_disable_ocr, 0),
    )?;
    class.define_method(
    ...
    class.define_singleton_method("new", function!(ImageExtractionConfig::new, 7))?;
    ...
    class.define_singleton_method("new", function!(OcrConfig::new, 12))?;
    ...
    class.define_method("validate", method!(OcrConfig::validate, 0))?;
    class.define_method("effective_thresholds", method!(OcrConfig::effective_thresholds, 0))?;
    class.define_method("effective_pipeline", method!(OcrConfig::effective_pipeline, 0))?;
    ...
    class.define_method("build_lookup_sets", method!(PostProcessorConfig::build_lookup_sets, 0))?;
    ...
    class.define_method("with_chunker_type", method!(ChunkingConfig::with_chunker_type, 1))?;
    class.define_method("with_sizing", method!(ChunkingConfig::with_sizing, 1))?;
    class.define_method(
        "with_prepend_heading_context",
        method!(ChunkingConfig::with_prepend_heading_context, 1),
    )?;
    class.define_method("with_topic_threshold", method!(ChunkingConfig::with_topic_threshold, 1))?;
    ...
    class.define_method("apply_env_overrides", method!(ServerConfig::apply_env_overrides, 0))?;
    ...
    class.define_method("read_u32", method!(StreamReader::read_u32, 0))?;
    ...
    class.define_method("current_size", method!(StringGrowthValidator::current_size, 0))?;

    let class = module.define_class("IterationValidator", ruby.class_object())?;
    class.define_method("check_iteration", method!(IterationValidator::check_iteration, 0))?;
    class.define_method("current_count", method!(IterationValidator::current_count, 0))?;

    let class = module.define_class("DepthValidator", ruby.class_object())?;
    class.define_method("push", method!(DepthValidator::push, 0))?;
    class.define_method("pop", method!(DepthValidator::pop, 0))?;
    class.define_method("current_depth", method!(DepthValidator::current_depth, 0))?;

    let class = module.define_class("EntityValidator", ruby.class_object())?;
    class.define_method("validate", method!(EntityValidator::validate, 1))?;

    let class = module.define_class("TableValidator", ruby.class_object())?;
    ...
    class.define_method(
        "avg_non_whitespace",
        method!(OcrFallbackDecision::avg_non_whitespace, 0),
    )?;
    ...
    class.define_method("push_node", method!(DocumentStructure::push_node, 1))?;
    class.define_method("add_child", method!(DocumentStructure::add_child, 2))?;
    ...
    class.define_method("with_max_keywords", method!(KeywordConfig::with_max_keywords, 1))?;
    class.define_method("with_min_score", method!(KeywordConfig::with_min_score, 1))?;
    class.define_method("with_ngram_range", method!(KeywordConfig::with_ngram_range, 2))?;
    class.define_method("with_language", method!(KeywordConfig::with_language, 1))?;
    ...
    class.define_method("width", method!(BBox::width, 0))?;
    ...
    class.define_method("fmt", method!(LayoutDetection::fmt, 1))?;
    ...
    let class = module.define_class("FontSizeCluster", ruby.class_object())?;
    class.define_singleton_method("new", function!(FontSizeCluster::new, 2))?;
    class.define_method("centroid", method!(FontSizeCluster::centroid, 0))?;
    class.define_method("members", method!(FontSizeCluster::members, 0))?;

    let class = module.define_class("CharData", ruby.class_object())?;
    class.define_singleton_method("new", function!(CharData::new, 9))?;
    class.define_method("text", method!(CharData::text, 0))?;
    class.define_method("x", method!(CharData::x, 0))?;
    class.define_method("y", method!(CharData::y, 0))?;
    class.define_method("font_size", method!(CharData::font_size, 0))?;
    class.define_method("width", method!(CharData::width, 0))?;
    class.define_method("height", method!(CharData::height, 0))?;
    class.define_method("is_bold", method!(CharData::is_bold, 0))?;
    ...
    module.define_module_function("smart_cleanup_cache", function!(smart_cleanup_cache, 4))?;
    module.define_module_function("is_cache_valid", function!(is_cache_valid, 2))?;
    module.define_module_function("clear_cache_directory", function!(clear_cache_directory, 1))?;
    module.define_module_function("batch_cleanup_caches", function!(batch_cleanup_caches, 4))?;
    ...
    module.define_module_function("get_available_disk_space", function!(get_available_disk_space, 1))?;
    ...
    module.define_module_function("filter_old_cache_entries", function!(filter_old_cache_entries, 3))?;
    module.define_module_function("sort_cache_by_access_time", function!(sort_cache_by_access_time, 1))?;
    module.define_module_function("sanitize_namespace", function!(sanitize_namespace, 1))?;
    module.define_module_function("is_batch_mode", function!(is_batch_mode, 0))?;
    module.define_module_function("resolve_thread_budget", function!(resolve_thread_budget, 1))?;
    module.define_module_function("init_thread_pools", function!(init_thread_pools, 1))?;
    module.define_module_function("merge_config_json", function!(merge_config_json, 2))?;
    module.define_module_function("build_config_from_json", function!(build_config_from_json, 2))?;
    ...
    module.define_module_function("validate_vlm_backend_config", function!(validate_vlm_backend_config, 2))?;
    module.define_module_function(
        "validate_structured_extraction_schema",
        function!(validate_structured_extraction_schema, 2),
    )?;
    ...
    module.define_module_function("find_files_by_extension", function!(find_files_by_extension, 3))?;
    ...
    module.define_module_function("parse_toml", function!(parse_toml, 1))?;
    module.define_module_function("parse_text", function!(parse_text, 2))?;
    module.define_module_function(
        "transform_to_document_structure",
        function!(transform_to_document_structure, 1),
    )?;
    module.define_module_function("detect_list_items", function!(detect_list_items, 1))?;
    module.define_module_function("generate_element_id", function!(generate_element_id, 3))?;
    module.define_module_function(
    ...
    module.define_module_function(
        ...
        function!(estimate_html_markdown_capacity, 1),
    )?;
    module.define_module_function(
        "estimate_spreadsheet_capacity",
        function!(estimate_spreadsheet_capacity, 1),
    )?;
    module.define_module_function(
        "estimate_presentation_capacity",
        function!(estimate_presentation_capacity, 1),
    )?;
    module.define_module_function(
        "estimate_table_markdown_capacity",
        function!(estimate_table_markdown_capacity, 2),
    )?;
    module.define_module_function("decompress_gzip", function!(decompress_gzip, 2))?;
    module.define_module_function("extract_gzip", function!(extract_gzip, 2))?;
    module.define_module_function("extract_gzip_metadata", function!(extract_gzip_metadata, 2))?;
    module.define_module_function("extract_gzip_text_content", function!(extract_gzip_text_content, 2))?;
    module.define_module_function("extract_gzip_with_bytes", function!(extract_gzip_with_bytes, 2))?;
    module.define_module_function("extract_7z_metadata", function!(extract_7z_metadata, 2))?;
    module.define_module_function("extract_7z_text_content", function!(extract_7z_text_content, 2))?;
    module.define_module_function("extract_7z_file_bytes", function!(extract_7z_file_bytes, 2))?;
    module.define_module_function("extract_tar_metadata", function!(extract_tar_metadata, 2))?;
    module.define_module_function("extract_tar_text_content", function!(extract_tar_text_content, 2))?;
    module.define_module_function("extract_tar_file_bytes", function!(extract_tar_file_bytes, 2))?;
    module.define_module_function("extract_zip_metadata", function!(extract_zip_metadata, 2))?;
    module.define_module_function("extract_zip_text_content", function!(extract_zip_text_content, 2))?;
    module.define_module_function("extract_zip_file_bytes", function!(extract_zip_file_bytes, 2))?;
    module.define_module_function("parse_eml_content", function!(parse_eml_content, 1))?;
    ...
    module.define_module_function(
        ...
        function!(convert_html_to_markdown_with_metadata, 3),
    )?;
    module.define_module_function(
        "convert_html_to_markdown_with_tables",
        function!(convert_html_to_markdown_with_tables, 3),
    )?;
    ...
    module.define_module_function("collect_and_convert_omath", function!(collect_and_convert_omath, 1))?;
    module.define_module_function("parse_document", function!(parse_document, 1))?;
    module.define_module_function("extract_text_from_bytes", function!(extract_text_from_bytes, 1))?;
    module.define_module_function("parse_section_properties", function!(parse_section_properties, 1))?;
    module.define_module_function(
    ...
    module.define_module_function(
        ...
        function!(extract_ppt_text_with_options, 2),
    )?;
    module.define_module_function("extract_pptx_from_path", function!(extract_pptx_from_path, 2))?;
    ...
    module.define_module_function("decode_iwa_stream", function!(decode_iwa_stream, 1))?;
    module.define_module_function("extract_text_from_proto", function!(extract_text_from_proto, 1))?;
    ...
    module.define_module_function("extract_rtf_metadata", function!(extract_rtf_metadata, 2))?;
    module.define_module_function("extract_rtf_formatting", function!(extract_rtf_formatting, 1))?;
    module.define_module_function("spans_to_annotations", function!(spans_to_annotations, 3))?;
    module.define_module_function("extract_text_from_rtf", function!(extract_text_from_rtf, 2))?;
    module.define_module_function("register_default_extractors", function!(register_default_extractors, 0))?;
    module.define_module_function("extract_panic_message", function!(extract_panic_message, 1))?;
    module.define_module_function("register_extractor", function!(register_extractor, 1))?;
    ...
    module.define_module_function("get_ocr_backend_registry", function!(get_ocr_backend_registry, 0))?;
    module.define_module_function(
        "get_document_extractor_registry",
        function!(get_document_extractor_registry, 0),
    )?;
    ...
    module.define_module_function("validate_plugins_at_startup", function!(validate_plugins_at_startup, 0))?;
    ...
    module.define_module_function("render_djot", function!(render_djot, 1))?;
    module.define_module_function("render_html", function!(render_html, 1))?;
    module.define_module_function("render_json", function!(render_json, 1))?;
    module.define_module_function("render_markdown", function!(render_markdown, 1))?;
    module.define_module_function("render_plain", function!(render_plain, 1))?;
    module.define_module_function("sanitize_filename", function!(sanitize_filename, 1))?;
    module.define_module_function("get_metrics", function!(get_metrics, 0))?;
    module.define_module_function(
        "record_error_on_current_span",
        function!(record_error_on_current_span, 1),
    )?;
    module.define_module_function(
        "record_success_on_current_span",
        function!(record_success_on_current_span, 0),
    )?;
    module.define_module_function("sanitize_path", function!(sanitize_path, 1))?;
    module.define_module_function("extractor_span", function!(extractor_span, 3))?;
    module.define_module_function("pipeline_stage_span", function!(pipeline_stage_span, 1))?;
    module.define_module_function("pipeline_processor_span", function!(pipeline_processor_span, 2))?;
    module.define_module_function("ocr_span", function!(ocr_span, 2))?;
    module.define_module_function("model_inference_span", function!(model_inference_span, 1))?;
    ...
    module.define_module_function("get_reduction_statistics", function!(get_reduction_statistics, 2))?;
    ...
    module.define_module_function("snake_to_camel", function!(snake_to_camel, 1))?;
    module.define_module_function("camel_to_snake", function!(camel_to_snake, 1))?;
    module.define_module_function("is_markdown_header", function!(is_markdown_header, 1))?;
    ...
    module.define_module_function("estimate_pool_size", function!(estimate_pool_size, 2))?;
    module.define_module_function("acquire_string_buffer", function!(acquire_string_buffer, 0))?;
    module.define_module_function("intern_language_code", function!(intern_language_code, 1))?;
    module.define_module_function("intern_mime_type", function!(intern_mime_type, 1))?;
    module.define_module_function("xml_tag_name", function!(xml_tag_name, 1))?;
    ...
    module.define_module_function(
        ...
        function!(create_router_with_limits_and_server_config, 3),
    )?;
    module.define_module_function("serve", function!(serve, 2))?;
    ...
    module.define_module_function(
        ...
        function!(serve_with_server_config_async, 2),
    )?;
    ...
    module.define_module_function("map_kreuzberg_error_to_mcp", function!(map_kreuzberg_error_to_mcp, 1))?;
    module.define_module_function("start_mcp_server", function!(start_mcp_server, 0))?;
    module.define_module_function("start_mcp_server_async", function!(start_mcp_server_async, 0))?;
    module.define_module_function(
        "start_mcp_server_with_config",
        function!(start_mcp_server_with_config, 1),
    )?;
    module.define_module_function(
    ...
    module.define_module_function(
        ...
        function!(detect_plain_text_boundaries, 1),
    )?;
    ...
    module.define_module_function("merge_segments", function!(merge_segments, 5))?;
    ...
    module.define_module_function(
        ...
        function!(register_language_detection_processor, 0),
    )?;
    ...
    module.define_module_function("ensure_ort_available", function!(ensure_ort_available, 0))?;
    ...
    module.define_module_function(
        ...
        function!(preprocess_imagenet_letterbox, 2),
    )?;
    ...
    module.define_module_function("take_or_create_tatr", function!(take_or_create_tatr, 1))?;
    module.define_module_function("return_tatr", function!(return_tatr, 1))?;
    module.define_module_function("take_or_create_slanet", function!(take_or_create_slanet, 2))?;
    module.define_module_function("return_slanet", function!(return_slanet, 2))?;
    module.define_module_function(
        "take_or_create_table_classifier",
        function!(take_or_create_table_classifier, 1),
    )?;
    module.define_module_function("return_table_classifier", function!(return_table_classifier, 1))?;
    module.define_module_function(
    ...
    module.define_module_function(
        ...
        function!(extract_metadata_with_password, 2),
    )?;
    module.define_module_function(
    ...
    module.define_module_function(
        ...
        function!(extract_common_metadata_from_document, 1),
    )?;
    module.define_module_function("render_page_to_image", function!(render_page_to_image, 3))?;
    module.define_module_function("render_pdf_page_to_png", function!(render_pdf_page_to_png, 4))?;
    module.define_module_function("extract_words_from_page", function!(extract_words_from_page, 2))?;
    ...
    module.define_module_function(
        ...
        function!(extract_text_from_pdf_with_password, 2),
    )?;
    module.define_module_function(
