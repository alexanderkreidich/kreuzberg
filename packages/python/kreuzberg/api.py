...
if TYPE_CHECKING:
    from ._kreuzberg import CharData, DetectionResult, DjotContent, EmailExtractionResult, ExcelWorkbook, FontSizeCluster, FormattedBlock, HeadingContext, InlineElement, LanguageDetectionConfig, LayoutDetection, PageBoundary, RecognizedTable
    from .options import AccelerationConfig, ChunkerType, ChunkingConfig, ContentFilterConfig, DocumentStructure, ElementType, EmailConfig, EmbeddingConfig, EmbeddingModelType, ExtractionConfig, ExtractionResult, HierarchyConfig, HtmlOutputConfig, ImageExtractionConfig, ImagePreprocessingConfig, KeywordConfig, LayoutDetectionConfig, LlmConfig, Metadata, OcrConfidence, OcrConfig, OcrElement, OcrElementConfig, OcrQualityThresholds, PageConfig, PdfConfig, PostProcessorConfig, RakeParams, ServerConfig, TesseractConfig, TokenReductionConfig, TreeSitterConfig, TreeSitterProcessConfig, YakeParams
...
_TO_RUST_CHUNKERTYPE_MAP = {
    ...
    "semantic": _rust.ChunkerType.Semantic,
}
...
def apply_output_format(result: ExtractionResult, output_format: str) -> _rust.ExtractionResult:
...
def is_markdown_header(line: str) -> bool:
    ...
    return _rust.is_markdown_header(line)
...
def detect_plain_text_boundaries(text: str) -> list[_rust.DetectedBoundary]:
    """Detect structural boundaries in plain text."""
    return _rust.detect_plain_text_boundaries(text)
...
def merge_segments(source_text: str, segments: list[str], boundaries: list[bool], max_characters: int, overlap: int) -> list[_rust.MergedChunk]:
    """Merge segments into chunks guided by topic boundaries."""
    return _rust.merge_segments(source_text, segments, boundaries, max_characters, overlap)


def chunk_semantic(text: str, config: ChunkingConfig, page_boundaries: list[PageBoundary] | None = None) -> _rust.ChunkingResult:
...
def take_or_create_slanet(variant: str, accel: AccelerationConfig | None = None) -> str | None:
    """Take a cached SLANeXT model for the given variant, or create a new one."""
    _rust_accel = _to_rust_acceleration_config(accel)
    return _rust.take_or_create_slanet(variant, _rust_accel)
...
def take_or_create_table_classifier(accel: AccelerationConfig | None = None) -> str | None:
    """Take a cached table classifier, or create a new one."""
    _rust_accel = _to_rust_acceleration_config(accel)
    return _rust.take_or_create_table_classifier(_rust_accel)
...
def extract_images_from_pdf_with_password(pdf_bytes: bytes, password: str) -> list[_rust.PdfImage]:
    """Extract images from pdf with password."""
...
def extract_metadata_with_passwords(pdf_bytes: bytes, passwords: list[str]) -> str:
    """Extract metadata with passwords."""
...
def extract_text_from_pdf_with_passwords(pdf_bytes: bytes, passwords: list[str]) -> str:
    """Extract text from pdf with passwords."""
