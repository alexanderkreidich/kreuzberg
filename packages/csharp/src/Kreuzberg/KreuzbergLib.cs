...
public static class KreuzbergLib
{
    ...
    public static ExtractionResult ApplyOutputFormat(ExtractionResult result, string outputFormat)
    {
    ...
    public static bool IsMarkdownHeader(string line)
    {
        ...
        var result = NativeMethods.IsMarkdownHeader(
            line
        );
        if (result == IntPtr.Zero) { var err = GetLastError(); if (err.Code != 0) throw err; }
        var returnValue = result != 0;
    ...
    {
        ArgumentNullException.ThrowIfNull(text);
        var result = NativeMethods.DetectPlainTextBoundaries(
            text
        );
        if (result == IntPtr.Zero) { var err = GetLastError(); if (err.Code != 0) throw err; }
        var json = Marshal.PtrToStringUTF8(result);
        NativeMethods.FreeString(result);
        var returnValue = JsonSerializer.Deserialize<List<DetectedBoundary>>(json ?? "null", JsonOptions)!;
        return returnValue;
    }
    ...
    /// <param name="maxCharacters"></param>
    /// <param name="overlap"></param>
    public static List<MergedChunk> MergeSegments(string sourceText, List<string> segments, List<bool> boundaries, ulong maxCharacters, ulong overlap)
    {
        ...
        var boundariesJson = JsonSerializer.Serialize(boundaries, JsonOptions);
        var boundariesHandle = Marshal.StringToHGlobalAnsi(boundariesJson);
        var result = NativeMethods.MergeSegments(
            sourceText,
            segmentsHandle,
            boundariesHandle,
            maxCharacters,
        ...
        var json = Marshal.PtrToStringUTF8(result);
        NativeMethods.FreeString(result);
        var returnValue = JsonSerializer.Deserialize<List<MergedChunk>>(json ?? "null", JsonOptions)!;
    ...
    public static List<LayoutDetection> ApplyHeuristics(List<LayoutDetection> detections, float pageWidth, float pageHeight)
    {
        ...
        var result = NativeMethods.ApplyHeuristics(
            detectionsHandle,
            pageWidth,
            pageHeight
        );
    ...
    public static string? TakeOrCreateTatr(AccelerationConfig? accel)
    {
    ...
    /// <param name="accel">Optional.</param>
    public static string? TakeOrCreateSlanet(string variant, AccelerationConfig? accel)
    {
    ...
    /// </summary>
    /// <param name="accel">Optional.</param>
    public static string? TakeOrCreateTableClassifier(AccelerationConfig? accel)
    {
        var accelJson = accel != null ? JsonSerializer.Serialize(accel, JsonOptions) : "null";
        var accelHandle = NativeMethods.AccelerationConfigFromJson(accelJson);
    ...
    public static ChunkingConfig ChunkingConfigWithTopicThreshold(float threshold)
    {
        var result = NativeMethods.ChunkingConfigWithTopicThreshold(
            threshold
        );
        var jsonPtr = NativeMethods.ChunkingConfigToJson(result);
        var json = Marshal.PtrToStringUTF8(jsonPtr);
