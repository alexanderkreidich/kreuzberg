...
## R API Reference <span class="version-badge">v4.9.1</span>
...
|------|------|----------|-------------|
| `cache_dir` | `character` | Yes | The cache dir |
...
```r
cleanup_cache(cache_dir, max_age_days, max_size_mb, target_size_ratio)
```
...
|------|------|----------|-------------|
| `cache_dir` | `character` | Yes | The cache dir |
| `max_age_days` | `numeric` | Yes | The max age days |
| `max_size_mb` | `numeric` | Yes | The max size mb |
| `target_size_ratio` | `numeric` | Yes | The target size ratio |
...
```r
smart_cleanup_cache(cache_dir, max_age_days, max_size_mb, min_free_space_mb)
```
...
| `max_age_days` | `numeric` | Yes | The max age days |
| `max_size_mb` | `numeric` | Yes | The max size mb |
| `min_free_space_mb` | `numeric` | Yes | The min free space mb |
...
```r
is_cache_valid(cache_path, max_age_days)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `cache_path` | `character` | Yes | The cache path |
| `max_age_days` | `numeric` | Yes | The max age days |
...
```r
batch_cleanup_caches(cache_dirs, max_age_days, max_size_mb, min_free_space_mb)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `cache_dirs` | `list` | Yes | The cache dirs |
| `max_age_days` | `numeric` | Yes | The max age days |
| `max_size_mb` | `numeric` | Yes | The max size mb |
| `min_free_space_mb` | `numeric` | Yes | The min free space mb |
...
```r
filter_old_cache_entries(cache_times, current_time, max_age_seconds)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `cache_times` | `list` | Yes | The cache times |
| `current_time` | `numeric` | Yes | The current time |
| `max_age_seconds` | `numeric` | Yes | The max age seconds |
...
```r
sort_cache_by_access_time(entries)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `namespace` | `character` | Yes | The namespace |
...
| `override_json` | `character` | Yes | The override json |
...
|------|------|----------|-------------|
| `base` | `ExtractionConfig` | Yes | The extraction config |
| `override_json` | `character or NULL` | No | The override json |
...
```r
validate_vlm_backend_config(backend, vlm_config = NULL)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `backend` | `character` | Yes | The OCR backend name |
| `vlm_config` | `LlmConfig or NULL` | No | The optional VLM config to validate |
...
```r
validate_structured_extraction_schema(schema, llm_model)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `schema` | `list` | Yes | The JSON schema value to validate |
| `llm_model` | `character` | Yes | The LLM model string from the nested `LlmConfig` |
...
```r
get_pool_sizing_hint(file_size, mime_type)
```
...
```r
apply_output_format(result, output_format)
```
...
```r
transform_to_document_structure(result)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `text` | `character` | Yes | The text to search for list items |
...
```r
generate_element_id(text, element_type, page_number = NULL)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `text` | `character` | Yes | The element text content |
| `element_type` | `ElementType` | Yes | The semantic element type |
| `page_number` | `integer or NULL` | No | Optional page number for multi-page documents |
...
```r
extract_gzip_metadata(bytes, limits)
```
...
```r
extract_gzip_text_content(bytes, limits)
```
...
```r
extract_gzip_with_bytes(bytes, limits)
```
...
```r
extract_7z_metadata(bytes, limits)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `bytes` | `raw` | Yes | The 7z archive bytes |
...
```r
extract_7z_text_content(bytes, limits)
```
...
```r
extract_7z_file_bytes(bytes, limits)
```
...
```r
extract_tar_metadata(bytes, limits)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `bytes` | `raw` | Yes | The TAR archive bytes (can be compressed with gzip or bzip2) |
| `limits` | `SecurityLimits` | Yes | Security limits for archive extraction |
...
```r
extract_tar_text_content(bytes, limits)
```
...
```r
extract_tar_file_bytes(bytes, limits)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `bytes` | `raw` | Yes | The TAR archive bytes |
| `limits` | `SecurityLimits` | Yes | Security limits for archive extraction |
...
```r
extract_zip_metadata(bytes, limits)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `bytes` | `raw` | Yes | The ZIP archive bytes |
| `limits` | `SecurityLimits` | Yes | Security limits for archive extraction |
...
```r
extract_zip_text_content(bytes, limits)
```
...
```r
extract_zip_file_bytes(bytes, limits)
```
...
```r
convert_html_to_markdown(html, options = NULL, output_format = NULL)
```
...
| `options` | `ConversionOptions or NULL` | No | Optional conversion options; defaults will be used if None |
| `output_format` | `KreuzbergOutputFormat or NULL` | No | Optional output format; defaults to Markdown if None |
...
```r
convert_html_to_markdown_with_metadata(html, options = NULL, output_format = NULL)
```
...
```r
convert_html_to_markdown_with_tables(html, options = NULL, output_format = NULL)
```
...
| `output_format` | `KreuzbergOutputFormat or NULL` | No | The kreuzberg output format |
...
```r
extract_html_inline_images(html, options = NULL)
```
...
```r
detect_table_page_numbers(bytes)
```
...
```r
extract_complete_djot_content(events, metadata, tables)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `events` | `list` | Yes | The events |
| `metadata` | `Metadata` | Yes | The metadata |
| `tables` | `list` | Yes | The tables |
...
```r
extract_tables_from_events(events)
```
...
```r
extract_text_from_events(events)
```
...
```r
render_block_to_djot(block, indent_level)
```
...
```r
render_list_item(item, indent, marker)
```
...
```r
render_inline_content(elements)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `segment` | `character` | Yes | The segment |
...
```r
extract_rtf_metadata(rtf_content, extracted_text)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `rtf_content` | `character` | Yes | The rtf content |
| `extracted_text` | `character` | Yes | The extracted text |
...
```r
extract_rtf_formatting(content)
```
...
```r
spans_to_annotations(para_start, para_end, formatting)
```
...
```r
extract_text_from_rtf(content, plain)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `name` | `character` | Yes | Name of the extractor to unregister |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `name` | `character` | Yes | Name of the renderer to unregister |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `name` | `character` | Yes | Name of the validator to unregister |
...
```r
intern_language_code(lang_code)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `lang_code` | `character` | Yes | The language code to intern (e.g., "en", "es", "fr") |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `mime_type` | `character` | Yes | The MIME type string to intern |
...
```r
load_server_config(config_path = NULL)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `config_path` | `character or NULL` | No | Optional path to a ServerConfig file (TOML, YAML, or JSON) |
...
```r
create_router_with_limits_and_server_config(config, limits, server_config)
```
...
```r
serve_with_config(host, port, config)
```
...
```r
serve_with_config_and_limits(host, port, config, limits)
```
...
```r
map_kreuzberg_error_to_mcp(error)
```
...
```r
start_mcp_server_with_config(config)
```
...
```r
detect_plain_text_boundaries(text)
```
...
```r
merge_segments(source_text, segments, boundaries, max_characters, overlap)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `source_text` | `character` | Yes | The source text |
| `segments` | `list` | Yes | The segments |
| `boundaries` | `list` | Yes | The boundaries |
...
```r
detect_topic_boundaries(segment_texts, forced_boundaries, embedding_config, threshold)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `segment_texts` | `list` | Yes | The segment texts |
| `forced_boundaries` | `list` | Yes | The forced boundaries |
| `embedding_config` | `EmbeddingConfig` | Yes | The embedding config |
| `threshold` | `numeric` | Yes | The threshold |
...
```r
take_or_create_tatr(accel = NULL)
```
...
```r
take_or_create_slanet(variant, accel = NULL)
```
...
```r
take_or_create_table_classifier(accel = NULL)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `blocks` | `list` | Yes | Slice of TextBlock objects to cluster |
...
```r
assign_heading_levels_smart(clusters, min_heading_ratio, min_heading_gap)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `clusters` | `list` | Yes | Slice of FontSizeCluster objects (sorted by centroid descending) |
| `min_heading_ratio` | `numeric` | Yes | Minimum ratio of heading centroid to body centroid (e.g. 1.15) |
| `min_heading_gap` | `numeric` | Yes | Minimum absolute font-size difference in points (e.g. 1.5) |
...
```r
assign_hierarchy_levels(blocks, kmeans_result)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `blocks` | `list` | Yes | Slice of TextBlock objects to assign hierarchy levels to |
...
```r
assign_hierarchy_levels_from_clusters(blocks, clusters)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `blocks` | `list` | Yes | Vector of TextBlock objects to assign levels to |
| `clusters` | `list` | Yes | Vector of FontSizeCluster objects from clustering |
...
```r
extract_segments_from_page(page)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `chars` | `list` | Yes | Vector of CharData to merge into blocks |
...
| `blocks` | `list` | Yes | Slice of TextBlock objects present on the page |
...
```r
from_mb(max_request_body_mb, max_multipart_field_mb)
```
...
| `v_merge` | `VerticalMerge or NULL` | `NULL` | V merge (vertical merge) |
...
| `config` | `ChunkingConfigRequest or NULL` | `NULL` | Optional chunking configuration |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `character` | — | Color scheme name. |
| `dk1` | `ThemeColor or NULL` | `NULL` | Dark 1 (dark background) color. |
| `lt1` | `ThemeColor or NULL` | `NULL` | Light 1 (light background) color. |
| `dk2` | `ThemeColor or NULL` | `NULL` | Dark 2 color. |
| `lt2` | `ThemeColor or NULL` | `NULL` | Light 2 color. |
| `accent1` | `ThemeColor or NULL` | `NULL` | Accent color 1. |
| `accent2` | `ThemeColor or NULL` | `NULL` | Accent color 2. |
| `accent3` | `ThemeColor or NULL` | `NULL` | Accent color 3. |
| `accent4` | `ThemeColor or NULL` | `NULL` | Accent color 4. |
| `accent5` | `ThemeColor or NULL` | `NULL` | Accent color 5. |
| `accent6` | `ThemeColor or NULL` | `NULL` | Accent color 6. |
| `hlink` | `ThemeColor or NULL` | `NULL` | Hyperlink color. |
| `fol_hlink` | `ThemeColor or NULL` | `NULL` | Followed hyperlink color. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `preprocess_ms` | `numeric` | — | Time spent in image preprocessing (resize, letterbox, normalize, tensor allocation). |
| `onnx_ms` | `numeric` | — | Time for the ONNX `session.run()` call (actual neural network computation). |
| `model_total_ms` | `numeric` | — | Total time from start of model call to end of raw output decoding. |
| `postprocess_ms` | `numeric` | — | Time spent in postprocessing heuristics (confidence filtering, overlap resolution). |
...
```r
with_acceleration(cache_dir, accel)
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `md_content` | `character` | — | Markdown content of the converted document |
...
| `acceleration` | `AccelerationConfig or NULL` | `NULL` | Hardware acceleration for the embedding ONNX model. When set, controls which execution provider (CPU, CUDA, CoreML, TensorRT) is used for inference. Defaults to `None` (auto-select per platform). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `error_type` | `character` | — | Error type name |
| `message` | `character` | — | Error message |
| `traceback` | `character or NULL` | `NULL` | Stack trace (if available) |
...
| `description` | `character or NULL` | `NULL` | Human-readable description |
| `dimensions` | `U32U32 or NULL` | `NULL` | Dimensions ((u32, u32)) |
...
| `cancel_token` | `CancellationToken or NULL` | `NULL` | Cancellation token for this extraction (None = no external cancellation). Pass a `CancellationToken` clone here and call `CancellationToken.cancel` from another thread / task to abort the extraction in progress. The extractor checks the token at safe checkpoints (before lock acquisition, between pages, between batch items) and returns `KreuzbergError.Cancelled` when set. The field is excluded from serialization because `CancellationToken` is a runtime handle, not a configuration value. |
...
| `result_format` | `OutputFormat or NULL` | `NULL` | Override result format for this file. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `character` | — | Font scheme name. |
...
| `plugins` | `PluginStatus or NULL` | `NULL` | Plugin status (optional) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `labels` | `list` | — | Cluster label for each block (0-indexed) |
...
| `acceleration` | `AccelerationConfig or NULL` | `NULL` | Hardware acceleration for ONNX models (layout detection + table structure). When set, controls which execution provider (CPU, CUDA, CoreML, TensorRT) is used for inference. Defaults to `None` (auto-select per platform). |
...
| `acceleration` | `AccelerationConfig or NULL` | `NULL` | Hardware acceleration for ONNX inference. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `relative_path` | `character` | — | Relative path within the cache directory (e.g., "paddle-ocr/det/model.onnx"). |
| `sha256` | `character` | — | SHA256 checksum of the model file. |
| `size_bytes` | `integer` | — | Expected file size in bytes. |
| `source_url` | `character` | — | HuggingFace source URL for downloading. |
...
| `alnum_ratio` | `numeric` | — | Alnum ratio |
| `garbage_char_count` | `integer` | — | Count of Unicode replacement characters (U+FFFD) indicating encoding failures. |
| `fragmented_word_ratio` | `numeric` | — | Fraction of whitespace-delimited words that are 1-2 characters (0.0-1.0). High values indicate fragmented/garbled text extraction. |
| `consecutive_repeat_ratio` | `numeric` | — | Fraction of consecutive word pairs that are identical (0.0-1.0). High values indicate column scrambling where text is duplicated. |
| `avg_word_length` | `numeric` | — | Average word length (by chars). Very low values indicate garbled extraction. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `source` | `character` | — | Original filename |
...
| `list_level` | `integer or NULL` | `NULL` | List nesting level (0-based). `None` means not a list item. |
| `list_id` | `integer or NULL` | `NULL` | List override ID (\lsN). Used to detect list boundaries. |
| `is_table` | `logical` | — | Whether this paragraph is a table placeholder (text is in tables vec). |
| `ordered` | `logical` | — | Whether this list item is ordered (numbered/lettered). Detected from `\listtext` or `\pntext` content. `False` = unordered (bullet). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model_dir` | `character` | — | Directory containing model.onnx. |
| `dict_file` | `character` | — | Path to the character dictionary file. |
| `model_key` | `character` | — | Engine pool key for sharing engines across script families. Multiple families may share the same key (e.g. chinese and japanese both map to "v2:unified_server" when using server tier). |
...
| `is_header` | `logical` | — | Whether header |
| `cant_split` | `logical` | — | Cant split |
...
| `strikethrough` | `logical` | — | Strikethrough |
| `subscript` | `logical` | — | Subscript |
| `superscript` | `logical` | — | Superscript |
...
| `orientation` | `Orientation or NULL` | `NULL` | Page orientation (from `w:pgSz w:orient`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text` | `character` | — | The segment text content (may contain spaces / multiple words) |
| `x` | `numeric` | — | Left x position in PDF units |
| `y` | `numeric` | — | Bottom y position in PDF units (PDF coordinate system, y=0 at bottom) |
| `width` | `numeric` | — | Width of the segment bounding box |
| `height` | `numeric` | — | Height of the segment bounding box |
| `font_size` | `numeric` | — | Font size in points (from first character) |
| `is_bold` | `logical` | — | Whether the font is bold |
| `is_italic` | `logical` | — | Whether the font is italic |
| `is_monospace` | `logical` | — | Whether the font is monospace (e.g. Courier, Consolas) |
| `baseline_y` | `numeric` | — | Baseline Y position (from first character origin, falls back to bounds bottom) |
| `assigned_role` | `integer or NULL` | `NULL` | Pre-assigned heading level from the PDF structure tree (1-6), or `None` when the heading level is unknown and must be inferred via font-size clustering. |
...
| `hit_rate` | `numeric` | — | Hit rate as percentage (0.0-100.0) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `first_row` | `logical` | — | First row |
| `last_row` | `logical` | — | Last row |
| `first_column` | `logical` | — | First column |
| `last_column` | `logical` | — | Last column |
| `no_h_band` | `logical` | — | No h band |
| `no_v_band` | `logical` | — | No v band |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `character` | — | Theme name (e.g., "Office Theme"). |
...
```r
from_file(path, variant, input_width, input_height, model_name, accel)
```
...
| Value | Description |
|-------|-------------|
| `invalid_format` | The file does not match the HWP 5.0 format. — Fields: `0`: `character` |
| `unsupported_version` | The HWP version or a feature is not supported (e.g. password-encrypted docs). — Fields: `0`: `character` |
| `io` | An underlying I/O error occurred. — Fields: `0`: `Error` |
| `cfb` | A CFB compound-file error (stream not found, corrupt container, etc.). — Fields: `0`: `character` |
| `compression_error` | Decompression of a zlib/deflate stream failed. — Fields: `0`: `character` |
| `parse_error` | The binary record stream could not be parsed. — Fields: `0`: `character` |
| `encoding_error` | A UTF-16LE string contained invalid data. — Fields: `0`: `character` |
| `not_found` | A requested stream was not present in the compound file. — Fields: `0`: `character` |
...
| Value | Description |
|-------|-------------|
| `run` | Plain text from m:r/m:t — Fields: `0`: `character` |
| `s_sup` | Superscript: base^{sup} — Fields: `base`: `list`, `sup`: `list` |
| `s_sub` | Subscript: base_{sub} — Fields: `base`: `list`, `sub`: `list` |
| `s_sub_sup` | Sub-superscript: base_{sub}^{sup} — Fields: `base`: `list`, `sub`: `list`, `sup`: `list` |
| `frac` | Fraction: \frac{num}{den} — Fields: `num`: `list`, `den`: `list`, `frac_type`: `FracType` |
| `rad` | Radical: \sqrt{body} or \sqrt[deg]{body} — Fields: `deg`: `list`, `body`: `list`, `deg_hide`: `logical` |
| `nary` | N-ary operator: \sum_{sub}^{sup}{body} — Fields: `chr`: `character`, `sub`: `list`, `sup`: `list`, `body`: `list`, `sub_hide`: `logical`, `sup_hide`: `logical` |
| `delim` | Delimiter: \left( ... \right) — Fields: `begin_chr`: `character`, `end_chr`: `character`, `sep_chr`: `character`, `elements`: `list` |
| `func` | Function: \funcname{body} — Fields: `name`: `list`, `body`: `list` |
| `acc` | Accent: \hat{body} — Fields: `chr`: `character`, `body`: `list` |
| `eq_arr` | Equation array: \begin{aligned}...\end{aligned} — Fields: `rows`: `list` |
| `lim_low` | Lower limit: \underset{lim}{body} — Fields: `body`: `list`, `lim`: `list` |
| `lim_upp` | Upper limit: \overset{lim}{body} — Fields: `body`: `list`, `lim`: `list` |
| `bar` | Bar (overline/underline) — Fields: `body`: `list`, `top`: `logical` |
| `border_box` | Border box: \boxed{body} — Fields: `body`: `list` |
| `matrix` | Matrix: \begin{matrix}...\end{matrix} — Fields: `rows`: `list` |
| `group` | Grouping container (m:box, m:phant, etc.) — passes through children — Fields: `children`: `list` |
| `s_pre` | Pre-sub-superscript: {}_{sub}^{sup}{base} — Fields: `base`: `list`, `sub`: `list`, `sup`: `list` |
...
| Value | Description |
|-------|-------------|
| `rgb` | Direct hex RGB color (e.g., "156082"). — Fields: `0`: `character` |
| `system` | System color with fallback RGB (e.g., "windowText" with lastClr "000000"). — Fields: `name`: `character`, `last_color`: `character` |
...
| Value | Description |
|-------|-------------|
| `zip_bomb_detected` | Potential ZIP bomb detected — Fields: `compressed_size`: `integer`, `uncompressed_size`: `integer`, `ratio`: `numeric` |
...
| `list_item` | List item. `ordered` indicates numbered vs bulleted. — Fields: `ordered`: `logical` |
...
| `list_start` | Start of a list container. — Fields: `ordered`: `logical` |
...
| `key` | Unresolved: key to be matched against element anchors during derivation. — Fields: `0`: `character` |
...
| Value | Description |
|-------|-------------|
| `file` | Extract from a filesystem path with an optional MIME type hint. — Fields: `path`: `character`, `mime_hint`: `character` |
| `bytes` | Extract from in-memory bytes with a known MIME type. — Fields: `data`: `raw`, `mime_type`: `character` |
...
| Value | Description |
|-------|-------------|
| `tokenizer` | Tokenizer — Fields: `0`: `character` |
| `ort` | Ort — Fields: `0`: `Error` |
| `shape` | Shape — Fields: `0`: `character` |
| `no_output` | No output |
...
| Value | Description |
|-------|-------------|
| `tesseract_initialization_failed` | Tesseract initialization failed — Fields: `0`: `character` |
| `unsupported_version` | Unsupported version — Fields: `0`: `character` |
| `invalid_configuration` | Invalid configuration — Fields: `0`: `character` |
| `invalid_language_code` | Invalid language code — Fields: `0`: `character` |
| `image_processing_failed` | Image processing failed — Fields: `0`: `character` |
| `processing_failed` | Processing failed — Fields: `0`: `character` |
| `cache_error` | Cache error — Fields: `0`: `character` |
| `io_error` | I o error — Fields: `0`: `character` |
...
| `custom` | Custom model from a local file path. — Fields: `path`: `character`, `variant`: `CustomModelVariant` |
...
| Value | Description |
|-------|-------------|
| `invalid_pdf` | Invalid pdf — Fields: `0`: `character` |
| `password_required` | Password required |
| `invalid_password` | Invalid password |
| `encryption_not_supported` | Encryption not supported — Fields: `0`: `character` |
| `page_not_found` | Page not found — Fields: `0`: `integer` |
| `text_extraction_failed` | Text extraction failed — Fields: `0`: `character` |
| `rendering_failed` | Rendering failed — Fields: `0`: `character` |
| `metadata_extraction_failed` | Metadata extraction failed — Fields: `0`: `character` |
| `extraction_failed` | Extraction failed — Fields: `0`: `character` |
| `font_loading_failed` | Font loading failed — Fields: `0`: `character` |
| `io_error` | I o error — Fields: `0`: `character` |
