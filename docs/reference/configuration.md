...
| `cancel_token` | `CancellationToken | None` | `None` | Cancellation token for this extraction (None = no external cancellation). Pass a `CancellationToken` clone here and call `CancellationToken.cancel` from another thread / task to abort the extraction in progress. The extractor checks the token at safe checkpoints (before lock acquisition, between pages, between batch items) and returns `KreuzbergError.Cancelled` when set. The field is excluded from serialization because `CancellationToken` is a runtime handle, not a configuration value. |
...
| `result_format` | `OutputFormat | None` | `None` | Override result format for this file. |
...
| `orientation` | `Orientation | None` | `None` | Page orientation (from `w:pgSz w:orient`). |
...
| `v_merge` | `VerticalMerge | None` | `None` | V merge (vertical merge) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `str` | — | Color scheme name. |
| `dk1` | `ThemeColor | None` | `None` | Dark 1 (dark background) color. |
| `lt1` | `ThemeColor | None` | `None` | Light 1 (light background) color. |
| `dk2` | `ThemeColor | None` | `None` | Dark 2 color. |
| `lt2` | `ThemeColor | None` | `None` | Light 2 color. |
| `accent1` | `ThemeColor | None` | `None` | Accent color 1. |
| `accent2` | `ThemeColor | None` | `None` | Accent color 2. |
| `accent3` | `ThemeColor | None` | `None` | Accent color 3. |
| `accent4` | `ThemeColor | None` | `None` | Accent color 4. |
| `accent5` | `ThemeColor | None` | `None` | Accent color 5. |
| `accent6` | `ThemeColor | None` | `None` | Accent color 6. |
| `hlink` | `ThemeColor | None` | `None` | Hyperlink color. |
| `fol_hlink` | `ThemeColor | None` | `None` | Followed hyperlink color. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `str` | — | Theme name (e.g., "Office Theme"). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `max_archive_size` | `int` | — | Maximum uncompressed size for archives (500 MB) |
...
| `max_content_size` | `int` | — | Maximum string growth per document (100 MB) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `non_whitespace` | `int` | — | Non whitespace |
| `alnum` | `int` | — | Alnum |
| `meaningful_words` | `int` | — | Meaningful words |
| `alnum_ratio` | `float` | — | Alnum ratio |
| `garbage_char_count` | `int` | — | Count of Unicode replacement characters (U+FFFD) indicating encoding failures. |
| `fragmented_word_ratio` | `float` | — | Fraction of whitespace-delimited words that are 1-2 characters (0.0-1.0). High values indicate fragmented/garbled text extraction. |
| `consecutive_repeat_ratio` | `float` | — | Fraction of consecutive word pairs that are identical (0.0-1.0). High values indicate column scrambling where text is duplicated. |
| `avg_word_length` | `float` | — | Average word length (by chars). Very low values indicate garbled extraction. |
| `word_count` | `int` | — | Total word count (whitespace-delimited). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `heading_level` | `int` | — | Heading level (1-based): 1 = H1, 2 = H2, etc. 0 = not a heading. |
| `list_level` | `int | None` | `None` | List nesting level (0-based). `None` means not a list item. |
| `list_id` | `int | None` | `None` | List override ID (\lsN). Used to detect list boundaries. |
...
| `links` | `list[StringString] | None` | `[]` | Markdown links as (text, url) tuples (for Markdown files) |
| `code_blocks` | `list[StringString] | None` | `[]` | Code blocks as (language, code) tuples (for Markdown files) |
...
| `custom_properties` | `dict[str, Any] | None` | `{}` | Custom properties from docProps/custom.xml (user-defined properties) Contains key-value pairs defined by users or applications. Values can be strings, numbers, booleans, or dates. |
...
| `column_count` | `int` | — | Number of column |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `entry_count` | `int` | — | Number of entry |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `citation_count` | `int` | — | Number of citation |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `record_count` | `int` | — | Number of record |
| `field_count` | `int` | — | Number of field |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `message_count` | `int` | — | Number of message |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `max_buffers_per_size` | `int` | `4` | Maximum buffers per size bucket |
| `initial_capacity` | `int` | `4096` | Initial capacity for new buffers |
| `max_capacity_before_discard` | `int` | `65536` | Maximum capacity before discarding |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `max_request_body_bytes` | `int` | — | Maximum size of the entire request body in bytes. This applies to the total size of all uploaded files and form data in a single request. Default: 100 MB (104,857,600 bytes). |
| `max_multipart_field_bytes` | `int` | — | Maximum size of a single multipart field in bytes. This applies to individual files in a multipart upload. Default: 100 MB (104,857,600 bytes). |
...
| `traceback` | `str | None` | `None` | Stack trace (if available) |
...
| `acceleration` | `AccelerationConfig | None` | `None` | Hardware acceleration for ONNX inference. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `preprocess_ms` | `float` | — | Time spent in image preprocessing (resize, letterbox, normalize, tensor allocation). |
| `onnx_ms` | `float` | — | Time for the ONNX `session.run()` call (actual neural network computation). |
| `model_total_ms` | `float` | — | Total time from start of model call to end of raw output decoding. |
| `postprocess_ms` | `float` | — | Time spent in postprocessing heuristics (confidence filtering, overlap resolution). |
