...
impl ServerConfig {
    ...
    #[allow(clippy::missing_errors_doc)]
    #[pyo3(signature = ())]
    pub fn apply_env_overrides(&self) -> PyResult<Self> {
...
impl DocumentStructure {
    ...
    pub fn add_child(&self, parent: u32, child: u32) -> Self {
        let mut core_self = kreuzberg::DocumentStructure {
            nodes: self.nodes.clone().into_iter().map(Into::into).collect(),
            source_format: self.source_format.clone(),
            relationships: self.relationships.clone().into_iter().map(Into::into).collect(),
...
pub fn extract_ooxml_embedded_objects<'py>(
    ...
    Err(pyo3::exceptions::PyNotImplementedError::new_err(
        "not implemented: extract_ooxml_embedded_objects",
    ))
}
...
pub fn dedup_text(texts: Vec<String>) -> Vec<String> {
    kreuzberg::extractors::iwork::dedup_text(texts).into_iter().collect()
}
...
pub fn list_renderers() -> Vec<String> {
    kreuzberg::plugins::list_renderers().into_iter().collect()
}
...
#[pyfunction]
#[pyo3(signature = (source_text, segments, boundaries, max_characters, overlap))]
...
#[pyfunction]
#[pyo3(signature = (variant, accel=None))]
...
pub fn extract_and_process_embedded_files<'py>(
    ...
    Err(pyo3::exceptions::PyNotImplementedError::new_err(
        "not implemented: extract_and_process_embedded_files",
    ))
}
...
fn kreuzberg_error_to_py_err(e: kreuzberg::error::KreuzbergError) -> pyo3::PyErr {
    ...
    match &e {
        ...
        kreuzberg::error::KreuzbergError::Cancelled => CancelledError::new_err(msg),
...
pub fn _kreuzberg(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(is_markdown_header, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(detect_plain_text_boundaries, m)?)?;
